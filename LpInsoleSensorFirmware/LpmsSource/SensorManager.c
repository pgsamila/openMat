...
uint8_t getSensorData(uint8_t* data, uint16_t *l)
{
	...
	if (connectedInterface == USB_CONNECTED) {
		...
		setI16t(&(data[o]), (int16_t) (forceSensorOutput[0] * (float) 0x7fff));
		o = o+2;
	
		setI16t(&(data[o]), (int16_t) (forceSensorOutput[1] * (float) 0x7fff));
		o = o+2;
	
		setI16t(&(data[o]), (int16_t) (forceSensorOutput[2] * (float) 0x7fff));
		o = o+2;
	
		setI16t(&(data[o]), (int16_t) (forceSensorOutput[3] * (float) 0x7fff));
		o = o+2;
        ...
        if ((gReg.data[LPMS_CONFIG] & LPMS_LPBUS_DATA_MODE_16BIT_ENABLED) != 0) {
                ...
                if ((gReg.data[LPMS_CONFIG] & LPMS_QUAT_OUTPUT_ENABLED) != 0) {
                        for (int i=0; i<4; i++) {
                                setFloat(&(data[i*4 + o]), forceSensorOutput[i] /* qAfterOffset.data[i] */, FLOAT_FULL_PRECISION);
                        }
                        o = o+16;
                }	
